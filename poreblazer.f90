...
module parameters
        ...
        integer :: nthreads_custom
        character*80      :: filenamedcd  !Meng
        Real(kind=RDbl)             :: zlow,zhigh                  ! lowedge
...
subroutine initialize(filename)
    ...
    ! Adjusting the position of the system in case it is seriously outside the simulation cell : Meng, change this, such that all frames are shifted the same way
...
subroutine lattice_calculations
    ...
    real*8              :: rdist2_surface_ref ! Meng
    real*8                             :: sigma, sigma6, sigma12,  sig2_rdist2, lj_energy
    integer*4                          :: i, j, k, l, icount, nstep,amin
    logical                            :: overlap
!----------------------- the following is the headers to read dcd -------------------------------

double precision  :: dimensions
real              :: dummyr
integer :: iframe
...
open(10,file=trim(filenamedcd),status='old',form='unformatted')
read(10) dummyc, nframes, (dummyi,i=1,8), dummyr, (dummyi,i=1,9)
read(10) dummyi, dummyr
read(10) natomdcd

if (natomdcd /= natoms ) then
   write(*,*) 'Error: dcd contains different number of atoms from the xyz file'
   STOP
end if
...
!    nthreads = 16
    CALL OMP_SET_NUM_THREADS(nthreads_custom)
...
!-------------- Read and offset the coordinates for each frame----------------------------------------
    read(10) (dimensions, j=1, 6)
    read(10) (coords(1, j),j=1,natoms)
    read(10) (coords(2, j),j=1,natoms)
    read(10) (coords(3, j),j=1,natoms)

    coords(1, :) = coords(1, :) - fcell%ell(1)
    coords(2, :) = coords(2, :) - fcell%ell(2)
    coords(3, :) = coords(3, :) - zlow

!    write(*,*) 'done reading the coordinates'

    call wrap_coordinate

!    write(*,*) 'done wrapping the coordinates'

    do i=1, natoms
        matvec(i)%comp = coords(:,i)
    end do

!    write(*,*) 'done assigning the coordinates'
!----------------------------------------------------------------------------------------

    ! sepvec is never used, but presumably giving each thread its own copy will increase performance.
    !$omp parallel do reduction( +: ng_cubes, nhe_cubes, nn_cubes ) private(overlap, icount, rdist2, rdist_surface, &
    !$omp rdist_surface_ref,rdist2_surface_ref, rdist2_ref, atvec1, atvec2, sepvec, amin)

    do l=1, ncubesz                                    ! we go cubelet by cubelet
        do k=1, ncubesy
            do j=1, ncubesx
!                write(*,*) "thread id: ",OMP_GET_THREAD_NUM() !Meng debug: output thread id
          ...
          !          end if
                ...
                rdist2_surface_ref = rdist_surface_ref*rdist_surface_ref
                if ( rdist2_surface_ref > lattice_rdist2(j,k,l) ) then
                    lattice_rdist2(j,k,l) = rdist2_surface_ref ! lattice_rdist2(j,k,l) stores the shortest squared distance between cubelet j, k, l and nearest atom (without overlap)
                end if

                if(rdist2_ref>asigma2_he(atype(amin))) then  ! next few lines detect if the cubelet is accessible to helium atom and update the list of
...
write(*,*) "--------ng_cubes------------",ng_cubes !Meng debug
...
!                icount = icount + 1
!                if (icount > ng_cubes) then
!                    write(*,*) "--------icount------------",icount !Meng debug
!                    STOP
!                end if
...
subroutine wrap_coordinate
    use adsorbent
    use parameters
    integer :: i,idimension
   
    do i = 1,natoms 
        do idimension = 1,2 
            do while (coords(idimension,i) < -0.5*fcell%ell(idimension))
                coords(idimension,i) = coords(idimension,i) + fcell%ell(idimension)
            end do
        end do
    end do
...
subroutine helium_lattice
    ...
    filename = "spanning_he.dat"
    filename1 = "cluster_info_he.dat"
    filename2 = "Pores_he.dat"
    ...
    call percolation_calc(lattice_space_he, he_cubes, nhe_cubes,cl_summary,filename,filename1,filename2)
...
subroutine nitrogen_lattice
    ...
    filename = "spanning_n.dat"
    filename1 = "cluster_info_n.dat"
    filename2 = "Pores_n.dat"
    ...
    call percolation_calc(lattice_space_n, n_cubes, nn_cubes,cl_summary,filename,filename1,filename2)
    ...
    do l=1, ncubesz
        do k=1, ncubesy
            do j=1, ncubesx
                if(lattice_space(j,k,l)<1) cycle
                icount = icount + 1
!                write(*,*) "output icount: ", icount
                PA1(icount) = lattice_rdist2(j,k,l)
                PA2(icount) = j
                PA3(icount) = k
                PA4(icount) = l
 !               write(*,*) "output icount: ", j,k,l
            end do
        end do
    end do
...
subroutine limiting_diameter
    ...
    filename1 = "cluster_info_ld.dat"
    filename2 = "Pores_ld.dat"
    ...
    do i=1, 100000 ! here we employ bisection method + percolation analysis to find the largest probe
        ...
        call percolation_calc_simple(lattice_temp, cl_summary, spanning,filename1,filename2) !Meng removed
        !call percolation_calc(lattice_temp, cl_summary, spanning) ! Meng
